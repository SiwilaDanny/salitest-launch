 ...
 * @param {object} [customSupabase=null] - Optional authenticated Supabase client
 * @returns {Promise<{success: boolean, message: string}>}
 */
export async function approveWalletTransaction(transactionId, approvalReason = 'Approved by admin', customSupabase = null) {
  try {
    const client = customSupabase || supabase;
    const { data, error } = await client.rpc('approve_wallet_transaction', {
 ...
 * @returns {Promise<{success: boolean, message: string}>}
 */
export async function rejectWalletTransaction(transactionId, rejectionReason, customSupabase = null) {
  try {
    const client = customSupabase || supabase;
    const { data, error } = await client.rpc('reject_wallet_transaction', {
      p_transaction_id: transactionId,
